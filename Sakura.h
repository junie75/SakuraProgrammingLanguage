...
// Sakura compiler "global" definitions and the common classes
//    SAKURAEXCEPTION, LISTER, READER, CODE, and IDENTIFIERTABLE
//
...
public:
   //-----------------------------------------------------------
   SAKURAEXCEPTION(const char description[])
   //-----------------------------------------------------------
   {
...
//-----------------------------------------------------------
{
   ...
   strcat(this->sourceFileName,".kura");
   strcpy(fullFileName,sourceFileName);
   strcat(fullFileName,".list");
   LIST.open(fullFileName,ios::out);
   if ( !LIST.is_open() ) throw( SAKURAEXCEPTION("Unable to open list file") );
   ListTopOfPageHeader();
}
...
*/
   ...
   time_t rawTime;
   struct tm * timeInfo;
   char sourceDateTime[80+1];
   
   time (&rawTime);
   timeInfo = localtime (&rawTime);
   sprintf(sourceDateTime, "%s", asctime(timeInfo));

   pageNumber++;
   LIST << FF << '"' << sourceFileName << "\" Page " << setw(4) << pageNumber << endl;
   //LIST << '"' << sourceFileName << "\" Page " << setw(4) << pageNumber << endl;
   LIST << sourceDateTime << endl;
...
//-----------------------------------------------------------
{
   ...
   strcpy(fullFileName,sourceFileName);
   strcat(fullFileName,".kura");
   SOURCE.open(fullFileName,ios::in);
   if ( !SOURCE.is_open() ) throw( SAKURAEXCEPTION("Unable to open source file") );
...
// index in [ 0,LOOKAHEAD ] where index = 0 means last GetNextCharacter() returned
   if ( (0 <= index) && (index <= LOOKAHEAD) )
      ...
      throw( SAKURAEXCEPTION("GetLookAheadCharacter() index out-of-range") );
}
...
//-----------------------------------------------------------
{
   if ( numberCallbacks <= CALLBACKSALLOWED )
      ...
      throw( SAKURAEXCEPTION("Too many callback functions") );
}
...
*/
   if ( identifiers > capacity )
      throw( SAKURAEXCEPTION("Identifier table capacity exceeded") );
   else
   {
      identifiers++;
...
//--------------------------------------------------
{
   ...
   if ( !STM.is_open() ) throw( SAKURAEXCEPTION("Unable to open code file") );
}
...
//--------------------------------------------------
{
   ...
   EmitUnformattedLine("");
   EmitUnformattedLine(";------------------------------------------------------------");
   EmitUnformattedLine("; Heap space for dynamic memory allocation (to support future Sakura syntax)");
   EmitUnformattedLine(";------------------------------------------------------------");
